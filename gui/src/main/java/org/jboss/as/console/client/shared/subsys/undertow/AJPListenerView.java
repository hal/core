...
public class AJPListenerView extends ModelDrivenWidget {
    ...
    public Widget buildWidget(ResourceAddress address, ResourceDefiniton definition) {
        ...
        tools.addToolButtonRight(new ToolButton(Console.CONSTANTS.common_label_delete(), new ClickHandler() {
            @Override
            public void onClick(ClickEvent event) {
                Feedback.confirm(Console.MESSAGES.deleteTitle("AJP Listener"),
                        Console.MESSAGES.deleteConfirm("AJP Listener '" + getCurrentSelection().getName() + "'"),
