...
public class ThreadPoolView {
    ...
    public Widget asWidget() {
        ...
        TextColumn<Property> enabledColumn = new TextColumn<Property>() {
            @Override
            public String getValue(Property node) {
                return String.valueOf(node.getValue().get("max-threads").asString());
            }
        };

        table.addColumn(nameColumn, "Name");
        table.addColumn(enabledColumn, "Max Threads");
        ...
        tools.addToolButtonRight(new ToolButton(Console.CONSTANTS.common_label_delete(), new ClickHandler() {
            @Override
            public void onClick(ClickEvent event) {
                Feedback.confirm(Console.MESSAGES.deleteTitle("Thread Pool"),
                        Console.MESSAGES.deleteConfirm("Thread Pool '" + getCurrentSelection().getName() + "'"),
